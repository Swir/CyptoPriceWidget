...
class PriceUpdater(QObject):
    ...
    def run(self):
        while self.running:
            ...
            time.sleep(40)  # Updated to update every minute
        ...
        for token in self.pinned_tokens:
            if token in data:
                ...
                if price is not None:
                    ...
                    price_info[token] = f"<font>{token}: <font style='color: {color};'>${price_str}{change_text}</font></font><br>"
                else:
                    price_info[token] = f"<font>{token}: <font style='color: white;'>No data</font></font><br>"
            else:
                price_info[token] = f"<font>{token}: <font style='color: white;'>No data</font></font><br>"
        return price_info
...
class CryptoPriceWidget(QWidget):
    def __init__(self):
        ...
        self.label = QLabel("Select cryptocurrency:")
    ...
    def update_price_label(self, price_info):
        ...
        self.price_label.setFont(QFont("Arial", 12))  # Change font size
    ...
    def animate_text(self):
        if self.current_index < len(self.full_text):
            ...
            self.current_text = ""
            self.current_index = 0
        QTimer.singleShot(20, self.animate_text)  # Update animation speed
